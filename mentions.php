...
<img src="<?php echo $location; ?>" width="200px" height="200px;" style="border-radius: 50%; border: 4px solid white; background-color: white; box-shadow: 0 8px 6px -6px black;
...
<i style="position: absolute; bottom: 1050px; left: 190px; font-size: 40px; color: #283e4a; background-color: white; border-color: 6px white; border-radius: 50%; border-width: 5px white; border-style: solid;
    border-width: 4px; border-color: white;" class="fas fa-check-circle"></i>

<?php }?>
 
<a style="color: black; font-size: 12px;" href="user_image.php">Update Image</a>
...
<hr>
    ...
    <?php $sql = "SELECT * FROM signup INNER JOIN posts ON signup.name = posts.name WHERE posts.post LIKE '%$username%' ORDER BY posts.id DESC";
$result = $conn->query($sql);
...
<div style="font-size: 12px;" class="text-muted"><i style="color: #2b6dad;" class="fas fa-user-tag"></i> <a class="text-muted" href="profile_view.php?name=<?php echo $row["name"]; ?>"><?php echo $row["name"];?></a> mentioned you</div>
...
?>     
     ...
     <?php } else {?>
        ...
        &nbsp;&nbsp;<a href="profile_view.php?name=<?php echo $row["name"]; ?>"><?php echo $row["name"];?></a>
...
<li class="nav-item">
    <a style="font-size: 15px; background-color: #e4e4e4; border-radius: 7px;" class="nav-link" href="profile.php"><img width="20px" height="20px" src="https://png.icons8.com/color/96/000000/email.png">&nbsp;Mentions<span style="font-size: 13px;" class="badge"><?php 
$id = isset($_SESSION['id']);
...
<p style="font-size: 17px; line-height: 5px;">Your Recent Blogs</p>
<hr>
...
<li class="nav-item" style="line-height: 30px; list-style-type: none;">
    ...
    <span class="text-muted" style="font-size: 12px;">(<?php echo time_ago_in_phpblog($btime); ?>)</span>
