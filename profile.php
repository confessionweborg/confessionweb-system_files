      ...
      <div class="modal-body">
        <form action="pos_update.php" method="post">
          <div class="form-group">
            ...
            <textarea name="position" type="text" id="editor2"><div class="item"><?php echo $row['position']; ?></div></textarea>
    <script>
      ClassicEditor
...
<hr>
    ...
    <?php $sql = "SELECT * FROM signup INNER JOIN posts ON signup.name = posts.name WHERE posts.name = '$username' ORDER BY posts.id DESC";
$result = $conn->query($sql);
...
echo $rowcounts;
  ...
  <li class="nav-item">
    <a style="font-size: 15px;" class="nav-link" href="mentions.php"><img width="20px" height="20px" src="https://png.icons8.com/color/96/000000/email.png">&nbsp;Mentions<span style="font-size: 13px;" class="badge"><?php 
$id = isset($_SESSION['id']);
...
$result = mysqli_query($conn, "SELECT SUM(VIEWS) AS totalviews FROM posts WHERE name = '$username' AND time >= NOW() + INTERVAL -7 DAY"); //7 Days ago views
...
if($sumviews > 1000)
 {$views_count=$sumviews *1/1000; 
 $views_kth=round($views_count, 2);



echo "     ".$views_kth."k";} else {echo $sumviews;} ?> Views last week</div>
